...
export default function Dashboard() {
  ...
  const [records, setRecords] = useState<any[]>([demoRecord, ...extraDemoRecords, olderDemoRecord]);
  ...
  const averageWeight = (records.reduce((sum, record) => sum + (record.weight || 0), 0) / records.length).toFixed(1);
  const bestCholesterol = Math.min(...records.map((record) => record.cholesterol_total || 999));
  const weightChange = latestRecord && records[records.length - 1]
    ? Number(latestRecord.weight) - Number(records[records.length - 1].weight)
    : 0;
  const heartRateChange = latestRecord && records[records.length - 1]
    ? Number(latestRecord.heart_rate) - Number(records[records.length - 1].heart_rate)
    : 0;
  const cholesterolChange = latestRecord && records[records.length - 1]
    ? Number(latestRecord.cholesterol_total) - Number(records[records.length - 1].cholesterol_total)
    : 0;
  ...
  useEffect(() => {
    const load = async () => {
      setUser({ id: 'demo-user', email: 'demo@my-med.app' });
  ...
  const handleUpload = async () => {
    ...
    alert('File upload is ready to connect to your PostgreSQL-backed storage flow once configured.');
  };
  ...
  const activePageSubtitle =
    activePage === 'summary'
      ...
      : activePage === 'vitals'
      ? 'Log today’s vitals in seconds'
      : activePage === 'workout'
        ? 'Stay active with a simple plan'
